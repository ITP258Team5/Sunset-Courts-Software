...
def inject_globals():
    now = datetime.now()
    return {
        'today_display': f"{now.strftime('%a %m/%d/%Y')} {now.hour % 12 or 12}:{now.strftime('%M %p')}"
    }
...
def report_monthly():
    ...
    total = query_db('''
        SELECT COUNT(*) as cnt
        FROM bookings
        WHERE booking_date >= ? AND booking_date < ? AND is_cancelled = 0
    ''', (start_date, end_date), one=True)['cnt']

    by_member = query_db('''
        SELECT f.family_name, COUNT(*) as cnt
        FROM bookings b
        JOIN families f ON b.family_id = f.family_id
        WHERE b.booking_date >= ? AND b.booking_date < ? AND b.is_cancelled = 0
        GROUP BY f.family_id, f.family_name
        ORDER BY cnt DESC, f.family_name
    ''', (start_date, end_date))
...
# ═══════════════════════════════════════════════════════════════════
@app.route('/reports/export/bookings')
def export_bookings_csv():
    rows = query_db('''
        SELECT b.booking_id, f.family_name, c.court_name,
               b.booking_date, b.start_time, b.end_time,
               b.guest_count, b.is_cancelled
        FROM bookings b
        JOIN families f ON b.family_id = f.family_id
        JOIN courts c ON b.court_id = c.court_id
    ...
    writer.writerow([
        'Booking ID', 'Account', 'Court',
        'Date', 'Start', 'End', 'Guests', 'Cancelled'
    ])
...
@app.route('/reports/export/members')
def export_members_csv():
    rows = query_db('''
        SELECT family_id, family_name, primary_contact, phone, email,
               join_date, is_banned, notes
        FROM families
    ...
    writer.writerow([
        'Family ID', 'Account Name', 'Primary Contact',
        'Phone', 'Email', 'Join Date', 'Banned', 'Notes'
    ])
...
@app.route('/reports/export/dues')
def export_dues_csv():
    rows = query_db('''
        SELECT f.family_name, d.year, d.is_paid, d.amount_paid, d.date_paid, d.notes
        FROM dues d
        JOIN families f ON d.family_id = f.family_id
        ORDER BY d.year DESC, f.family_name
    ''')
    ...
    writer.writerow([
        'Account Name', 'Year', 'Paid', 'Amount Paid', 'Date Paid', 'Notes'
    ])
